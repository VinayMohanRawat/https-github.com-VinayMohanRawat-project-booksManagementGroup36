...
const createBook = async function (req, res) {
    ...
    try {
        ...
        if (!(/^\d{4}-\d{2}-\d{2}$/.test(releasedAt))) { return res.status(400).send({ status: false, message: "not valid date format" }) }
    ...
    try {
        ...
        if (releasedAt) {
            ...
            if (!(/^\d{4}-\d{2}-\d{2}$/.test(releasedAt))) {
                 return res.status(400).send({ status: false, message: "not valid date format" }) 
                }
